...
$app->router->group('/api', [SecurityHeadersMiddleware::class, CorsMiddleware::class, 'version', 'etag', 'metrics', 'audit'], function (\Siro\Core\Router $router): void {
    ...
    // Generated by: php siro make:crud Product
    $router->get('/product', [\App\Controllers\ProductController::class, 'index']);
    $router->get('/product/{id}', [\App\Controllers\ProductController::class, 'show']);
    $router->post('/product', [\App\Controllers\ProductController::class, 'store'])
        ->middleware([JsonMiddleware::class]);
    $router->put('/product/{id}', [\App\Controllers\ProductController::class, 'update'])
        ->middleware([JsonMiddleware::class]);
    $router->delete('/product/{id}', [\App\Controllers\ProductController::class, 'delete'])
        ->middleware([JsonMiddleware::class]);
});
